 ...
 * four golden images covering the two required dimensions:
 *  - **light** / **dark** — both color themes (driven by the Robolectric `night` qualifier, which
 *    [com.sportygroup.weatherapp.core.designsystem.theme.SkyTheme] reads via `isSystemInDarkTheme`).
 *  - **a11yLargeFont light/dark** — a 1.5x font scale, the key visual accessibility regression
 *    check (verifies text scales without truncation or overlap).
 *
 * Record goldens with `recordRoborazziDebug`; verify with `verifyRoborazziDebug`.
 */
