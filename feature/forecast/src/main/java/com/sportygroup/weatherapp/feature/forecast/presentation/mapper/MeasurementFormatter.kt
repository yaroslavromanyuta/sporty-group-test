...
class MeasurementFormatter @Inject constructor(
    ...
    fun wind(windSpeed: Double, system: MeasurementSystem): FormattedMeasure = FormattedMeasure(
        ...
        unit = stringResources.getString(
            if (system == MeasurementSystem.IMPERIAL) R.string.unit_wind_mph else R.string.unit_wind_kmh,
        ),
    )
    ...
    fun pressure(pressureHpa: Double, system: MeasurementSystem): FormattedMeasure =
        if (system == MeasurementSystem.IMPERIAL) {
            FormattedMeasure(
                ...
                unit = stringResources.getString(R.string.unit_pressure_inhg),
            )
        } else {
            FormattedMeasure(
                value = pressureHpa.roundToInt().toString(),
                unit = stringResources.getString(R.string.unit_pressure_hpa),
            )
        }
}